...
// Initialize OpenAI (you can replace this with any AI service)
let openai = null;
let useSimpleAI = true; // Start with simple AI by default
...
try {
  if (process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY !== 'your-openai-api-key-here') {
    ...
    console.log('OpenAI API key not configured, using simple AI fallback');
  }
} catch (error) {
  console.log('OpenAI initialization failed, using simple AI fallback:', error.message);
}
...
DOCUMENT SERVICES:
- Form 137 (Student Permanent Record): ₱50, 3-5 business days
- SF9 (Student Report Card): ₱30, 2-3 business days  
- SF10 (Student Learner's Profile): ₱30, 3-4 business days
- Form 138 (Report Card): ₱30, 2-3 business days
- Diploma Copy: ₱100, 5-7 business days
...
const generateContextualResponse = async (message, intent, conversationHistory, userContext) => {
  ...
  if (!useSimpleAI && openai) {
    try {
      const contextMessages = [
        { role: 'system', content: schoolContext.systemPrompt },
  ...
  // Use simple AI (fallback or primary)
  const intelligentResponse = simpleAI.generateResponse(message, intent, conversationHistory, userContext);
...
exports.processMessage = async (req, res) => {
  try {
    ...
    // Classify intent
    const intent = useSimpleAI ? simpleAI.classifyIntent(sanitizedMessage) : classifyIntent(sanitizedMessage);
